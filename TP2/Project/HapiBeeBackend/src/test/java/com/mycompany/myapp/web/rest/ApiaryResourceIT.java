...
class ApiaryResourceIT {
    ...
    @Test
    @Transactional
    void getAllApiariesByZoneIsEqualToSomething() throws Exception {
        Zone zone;
        ...
        // Get all the apiaryList where zone equals to zoneId
        defaultApiaryShouldBeFound("zoneId.equals=" + zoneId);

        // Get all the apiaryList where zone equals to (zoneId + 1)
        defaultApiaryShouldNotBeFound("zoneId.equals=" + (zoneId + 1));
    }
    ...
    void partialUpdateApiaryWithPatch() throws Exception {
        ...
        partialUpdatedApiary.number(UPDATED_NUMBER).transhumance(UPDATED_TRANSHUMANCE);
