...
export class HivePage implements OnInit {
  ...
  async edit(item: IonItemSliding, hive: Hive) {
    let navigationExtras : NavigationExtras  = { state : { apiaryId: this.apiaryId }, queryParams : { apiaryId: this.apiaryId  } }
    await this.navController.navigateForward('/tabs/entities/hive/' + hive.id + '/edit', navigationExtras);
    await item.close();
  }
  ...
  async view(hive: Hive) {
    const navigationExtras: NavigationExtras  = { state : { apiaryId: this.apiaryId }, queryParams : { apiaryId: this.apiaryId  } }
    await this.navController.navigateForward('/tabs/entities/hive/' + hive.id + '/view', navigationExtras);
  }
}
