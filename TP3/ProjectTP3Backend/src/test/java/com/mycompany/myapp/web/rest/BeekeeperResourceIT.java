...
import com.mycompany.myapp.domain.enumeration.BeekeeperState;
...
class BeekeeperResourceIT {
    ...
    private static final BeekeeperState DEFAULT_STATE = BeekeeperState.REGISTERED;
    private static final BeekeeperState UPDATED_STATE = BeekeeperState.ANNULLED;
    ...
    void createBeekeeper() throws Exception {
        ...
        assertThat(testBeekeeper.getState()).isEqualTo(DEFAULT_STATE);
    }
    ...
    void checkStateIsRequired() throws Exception {
        int databaseSizeBeforeTest = beekeeperRepository.findAll().size();
        // set the field null
        beekeeper.setState(null);

        // Create the Beekeeper, which fails.
        BeekeeperDTO beekeeperDTO = beekeeperMapper.toDto(beekeeper);
        ...
        List<Beekeeper> beekeeperList = beekeeperRepository.findAll();
        assertThat(beekeeperList).hasSize(databaseSizeBeforeTest);
    }
    ...
    @Test
    @Transactional
    void getAllBeekeepersByStateIsEqualToSomething() throws Exception {
        // Initialize the database
        beekeeperRepository.saveAndFlush(beekeeper);

        // Get all the beekeeperList where state equals to DEFAULT_STATE
        defaultBeekeeperShouldBeFound("state.equals=" + DEFAULT_STATE);

        // Get all the beekeeperList where state equals to UPDATED_STATE
        defaultBeekeeperShouldNotBeFound("state.equals=" + UPDATED_STATE);
    }

    @Test
    @Transactional
    void getAllBeekeepersByStateIsInShouldWork() throws Exception {
        // Initialize the database
        beekeeperRepository.saveAndFlush(beekeeper);

        // Get all the beekeeperList where state in DEFAULT_STATE or UPDATED_STATE
        defaultBeekeeperShouldBeFound("state.in=" + DEFAULT_STATE + "," + UPDATED_STATE);

        // Get all the beekeeperList where state equals to UPDATED_STATE
        defaultBeekeeperShouldNotBeFound("state.in=" + UPDATED_STATE);
    }

    @Test
    @Transactional
    void getAllBeekeepersByStateIsNullOrNotNull() throws Exception {
        // Initialize the database
        beekeeperRepository.saveAndFlush(beekeeper);

        // Get all the beekeeperList where state is not null
        defaultBeekeeperShouldBeFound("state.specified=true");

        // Get all the beekeeperList where state is null
        defaultBeekeeperShouldNotBeFound("state.specified=false");
    }
    ...
    @Test
    @Transactional
    void getAllBeekeepersByTranshumanceRequestIsEqualToSomething() throws Exception {
        ...
        em.flush();
        beekeeper.addTranshumanceRequest(transhumanceRequest);
        beekeeperRepository.saveAndFlush(beekeeper);
        Long transhumanceRequestId = transhumanceRequest.getId();

        // Get all the beekeeperList where transhumanceRequest equals to transhumanceRequestId
        defaultBeekeeperShouldBeFound("transhumanceRequestId.equals=" + transhumanceRequestId);

        // Get all the beekeeperList where transhumanceRequest equals to (transhumanceRequestId + 1)
        defaultBeekeeperShouldNotBeFound("transhumanceRequestId.equals=" + (transhumanceRequestId + 1));
    }
    ...
    void putExistingBeekeeper() throws Exception {
        ...
        assertThat(testBeekeeper.getState()).isEqualTo(UPDATED_STATE);
    }
