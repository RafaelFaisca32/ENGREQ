...
class FrameResourceIT {
    ...
    @Test
    @Transactional
    void getAllFramesByCrestIsEqualToSomething() throws Exception {
        Crest crest;
        ...
        em.persist(crest);
        em.flush();
        frame.addCrest(crest);
        frameRepository.saveAndFlush(frame);
        Long crestId = crest.getId();

        // Get all the frameList where crest equals to crestId
        defaultFrameShouldBeFound("crestId.equals=" + crestId);

        // Get all the frameList where crest equals to (crestId + 1)
        defaultFrameShouldNotBeFound("crestId.equals=" + (crestId + 1));
    }

    @Test
    @Transactional
    void getAllFramesByUnfoldingRemovedFramesOldHiveIsEqualToSomething() throws Exception {
        Unfolding unfoldingRemovedFramesOldHive;
        if (TestUtil.findAll(em, Unfolding.class).isEmpty()) {
            frameRepository.saveAndFlush(frame);
            unfoldingRemovedFramesOldHive = UnfoldingResourceIT.createEntity(em);
        } else {
            unfoldingRemovedFramesOldHive = TestUtil.findAll(em, Unfolding.class).get(0);
        }
        em.persist(unfoldingRemovedFramesOldHive);
        em.flush();
        frame.addUnfoldingRemovedFramesOldHive(unfoldingRemovedFramesOldHive);
        frameRepository.saveAndFlush(frame);
        Long unfoldingRemovedFramesOldHiveId = unfoldingRemovedFramesOldHive.getId();

        // Get all the frameList where unfoldingRemovedFramesOldHive equals to unfoldingRemovedFramesOldHiveId
        defaultFrameShouldBeFound("unfoldingRemovedFramesOldHiveId.equals=" + unfoldingRemovedFramesOldHiveId);

        // Get all the frameList where unfoldingRemovedFramesOldHive equals to (unfoldingRemovedFramesOldHiveId + 1)
        defaultFrameShouldNotBeFound("unfoldingRemovedFramesOldHiveId.equals=" + (unfoldingRemovedFramesOldHiveId + 1));
    }

    @Test
    @Transactional
    void getAllFramesByUnfoldingInsertedFramesOldHiveIsEqualToSomething() throws Exception {
        Unfolding unfoldingInsertedFramesOldHive;
        if (TestUtil.findAll(em, Unfolding.class).isEmpty()) {
            frameRepository.saveAndFlush(frame);
            unfoldingInsertedFramesOldHive = UnfoldingResourceIT.createEntity(em);
        } else {
            unfoldingInsertedFramesOldHive = TestUtil.findAll(em, Unfolding.class).get(0);
        }
        em.persist(unfoldingInsertedFramesOldHive);
        em.flush();
        frame.addUnfoldingInsertedFramesOldHive(unfoldingInsertedFramesOldHive);
        frameRepository.saveAndFlush(frame);
        Long unfoldingInsertedFramesOldHiveId = unfoldingInsertedFramesOldHive.getId();

        // Get all the frameList where unfoldingInsertedFramesOldHive equals to unfoldingInsertedFramesOldHiveId
        defaultFrameShouldBeFound("unfoldingInsertedFramesOldHiveId.equals=" + unfoldingInsertedFramesOldHiveId);

        // Get all the frameList where unfoldingInsertedFramesOldHive equals to (unfoldingInsertedFramesOldHiveId + 1)
        defaultFrameShouldNotBeFound("unfoldingInsertedFramesOldHiveId.equals=" + (unfoldingInsertedFramesOldHiveId + 1));
    }

    @Test
    @Transactional
    void getAllFramesByUnfoldingInsertedFramesNewHiveIsEqualToSomething() throws Exception {
        Unfolding unfoldingInsertedFramesNewHive;
        if (TestUtil.findAll(em, Unfolding.class).isEmpty()) {
            frameRepository.saveAndFlush(frame);
            unfoldingInsertedFramesNewHive = UnfoldingResourceIT.createEntity(em);
        } else {
            unfoldingInsertedFramesNewHive = TestUtil.findAll(em, Unfolding.class).get(0);
        }
        em.persist(unfoldingInsertedFramesNewHive);
        em.flush();
        frame.addUnfoldingInsertedFramesNewHive(unfoldingInsertedFramesNewHive);
        frameRepository.saveAndFlush(frame);
        Long unfoldingInsertedFramesNewHiveId = unfoldingInsertedFramesNewHive.getId();

        // Get all the frameList where unfoldingInsertedFramesNewHive equals to unfoldingInsertedFramesNewHiveId
        defaultFrameShouldBeFound("unfoldingInsertedFramesNewHiveId.equals=" + unfoldingInsertedFramesNewHiveId);

        // Get all the frameList where unfoldingInsertedFramesNewHive equals to (unfoldingInsertedFramesNewHiveId + 1)
        defaultFrameShouldNotBeFound("unfoldingInsertedFramesNewHiveId.equals=" + (unfoldingInsertedFramesNewHiveId + 1));
    }
