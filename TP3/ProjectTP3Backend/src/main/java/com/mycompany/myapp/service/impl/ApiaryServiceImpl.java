...
public class ApiaryServiceImpl implements ApiaryService {
    ...
    @Override
    public void sendAuthorizationRequestToPortal(long id, ApiaryDTO apiaryDTO) throws IOException, InterruptedException {
        String postMapping = portalEndpoint + "api/apiary/" + id;
        HttpClient client = HttpClient.newHttpClient();

        ObjectMapper objectMapper = new ObjectMapper();
        objectMapper.registerModule(new JavaTimeModule());
        String requestBody = objectMapper.writeValueAsString(apiaryDTO);
